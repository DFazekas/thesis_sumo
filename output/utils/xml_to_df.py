...
def get_options(args=None):
  ...
  optParser.add_argument("--cols", dest="colNames", help="[Required] Comma-separated list of column names, in order (left to right).")
...
def parse(row, node, cols):
  ...
  for index, col in enumerate(cols):
    if node.attrib.get(col) is not None:
      log(f"\t\tAttr ({colored(col, 'green')}) found. Value ({colored(node.attrib.get(col), attrs=['bold'])})")
